...
const initializePlaybackLoop = () => {
  ...
  const availableAds = manifest.value.general_ads.filter(ad => canPlayVideo(ad))
  
  if (availableAds.length === 0) {
    log('warn', 'No available general ads with sufficient balance')
    if (manifest.value.default_video) {
  ...
  log('info', `Playback loop initialized with ${playbackLoop.value} items (${availableAds.length} ads)`)
}
