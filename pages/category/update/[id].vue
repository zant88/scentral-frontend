...
const getCategoryData = async () => {
  try {
    ...
    categoryId.value = id;
    const response = await fetchWithAuth(`${apiUrl}/api/category/${id}`, {
      method: 'GET',
      headers: {
    ...
    if (data.success) {
      ...
      categoryActive.value = category.is_active;
    } else {
      $toast.error('Failed to load category data', { duration: 5000, position: 'top-right' });
      navigateTo('/category');
    }
  } catch (error) {
    $toast.error('Failed to load category data', { duration: 5000, position: 'top-right' });
