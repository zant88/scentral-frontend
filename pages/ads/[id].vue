...
// Support both original route params and new query parameter structure
const machineId = route.params.id || route.query.device_id;
...
const isPlayingFromBrand = ref(false);
...
// Function to stop current video and play a new one
function stopAndPlayNewVideo(newAd, newIndex = null) {
  if (!videoPlayer.value || !newAd) return;
  
  console.log('Stopping current video and playing new video:', newAd.title);
  
  // Stop current video
  stopVideo();
  
  // Update current ad and index
  currentAd.value = newAd;
  if (newIndex !== null) {
    currentAdIndex.value = newIndex;
  }
  
  // Play new video
  playVideo();
  
  mqttStatus.value = `Now playing: ${newAd.title}`;
...
// function to get video from brand in advertisements
function getCurrentAdFromBrand(brandId) {
  if (advertisements.value.length === 0) return;
  const ad = advertisements.value.find(ad => ad.brand_id == brandId);
  if (ad) {
    currentAd.value = ad;
    currentAdIndex.value = advertisements.value.indexOf(ad);
    return true;
  }
  return false;
}
...
function nextAd() {
  ...
  if (isPlayingFromBrand.value) {
    isTransitioning.value = false;
    if (getCurrentAdFromBrand(brandId.value)) {
      stopAndPlayNewVideo(currentAd.value, currentAdIndex.value);
    }
    return;
  }
  ...
  if (advertisements.value.length === 1) {
    if (currentAd.value.video_url && videoPlayer.value) {
      stopAndPlayNewVideo(currentAd.value, 0);
    }
    isTransitioning.value = false;
    return;
  }
  ...
  // Stop current and play the next video
  if (currentAd.value.video_url && videoPlayer.value) {
    stopAndPlayNewVideo(currentAd.value, currentAdIndex.value);
  }
...
onMounted(() => {
  // Setup custom MQTT handler for this machine
  setupMqttHandler();
...
// Function to fetch brand-specific advertisements
async function fetchBrandAdvertisements(selectedBrandId) {
  try {
    console.log(`Fetching advertisements for brand ID: ${selectedBrandId}`);
    const response = await fetch(`${apiBase}/api/advertisement/brand/${selectedBrandId}`);
    
    if (!response.ok) {
      throw new Error(`Failed to fetch brand advertisements: ${response.status}`);
    }
    
    const brandAds = await response.json();
    console.log('Brand advertisements fetched:', brandAds);
    
    if (brandAds && brandAds.length > 0) {
      // Process video URLs for brand advertisements
      ...
      // Update advertisements list with brand-specific ads
      advertisements.value = brandAds;
      
      // Clear failed videos list for new playlist
      failedVideos.value.clear();
      
      // Stop current video and play first brand video
      if (brandAds[0] && brandAds[0].video_url && videoPlayer.value) {
        stopAndPlayNewVideo(brandAds[0], 0);
      }
      
      // Set brand playing flags
       isPlayingFromBrand.value = true;
       brandId.value = selectedBrandId;
      
      // Restart ad rotation with new playlist
      if (advertisements.value.length > 1) {
        startAdRotation();
      }
      
      mqttStatus.value = `Switched to brand playlist (${brandAds.length} videos)`;
      setTimeout(() => { mqttStatus.value = ''; }, 5000);
    }
  } catch (error) {
    console.error('Error fetching brand advertisements:', error);
    mqttStatus.value = 'Failed to load brand playlist';
    setTimeout(() => { mqttStatus.value = ''; }, 3000);
  }
}

// Function to fetch product brand information
async function fetchProductBrand(productId) {
  try {
    console.log(`Fetching brand for product ID: ${productId}`);
    const response = await fetch(`${apiBase}/api/product/${productId}`);
...
// Custom MQTT message handler for this ads page
function handleMqttMessage(topic, message, data) {
  console.log(`Custom MQTT handler - Topic: ${topic}, Machine: ${data.machine_code}`);
  
  // Check if the machine_code matches this page's ID
  if (data.machine_code === machineId) {
    // Check if the message contains brand information
    if (data.brand_id) {
      if (parseInt(data.brand_id) != -1) {
        ...
        console.log('this is from default reset');
        isPlayingFromBrand.value = false;
        if (advertisements.value.length > 0) {
          currentAd.value = advertisements.value[0];
          currentAdIndex.value = 0;
          stopAndPlayNewVideo(advertisements.value[0], 0);
        }
      }
      
    } else {
      // Handle other types of messages for this machine
      console.log('MQTT message received for this machine:', data);
      mqttStatus.value = `Machine state: ${JSON.stringify(data)}`;
      setTimeout(() => { mqttStatus.value = ''; }, 3000);
    }
  }
}
...
function setupMqttHandler() {
  try {
    // Register custom MQTT handler for this machine ID
    if ($mqtt && $mqtt.registerHandler) {
      $mqtt.registerHandler(machineId, handleMqttMessage);
      console.log(`Registered MQTT handler for machine: ${machineId}`);
      mqttStatus.value = 'MQTT handler registered';
      setTimeout(() => { mqttStatus.value = ''; }, 2000);
    } else {
      console.error('MQTT client or registerHandler method not available');
    }
  } catch (err) {
    console.error('Failed to setup MQTT handler:', err);
    mqttStatus.value = 'Failed to setup MQTT handler';
    setTimeout(() => { mqttStatus.value = ''; }, 3000);
  }
}
...
function cleanupMqttHandler() {
  try {
    // Unregister custom MQTT handler when component unmounts
    if ($mqtt && $mqtt.unregisterHandler) {
      $mqtt.unregisterHandler(machineId);
      console.log(`Unregistered MQTT handler for machine: ${machineId}`);
    }
  } catch (err) {
    console.error('Failed to cleanup MQTT handler:', err);
  }
}
